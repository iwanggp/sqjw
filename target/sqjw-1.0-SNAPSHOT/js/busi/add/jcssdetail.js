...
(function () {
    ...
    $dialog.attr("mask", "false");
    bringDialogToFront($dialog);
    initServiceParaSelect('za_jcss.sslx', $('#sslx', $dialog));
    $('#sslx', $dialog).attr("disabled", "disabled");
    ...
    $('#modify', $dialog).click(function () {
        var btns = new Array();
//        var mov = map.getOverlayById(param.hyid);
//        mov.moveable = true;
        if (f = !f) {
...
//                    mov.moveable = false;
            ...
            if ($("#shop_form", $dialog).valid()) {
                ...
                opt.put("service_code", "P23004");
                opt.put("id", param.hyid);
//                opt.put('jd', mov.point.x);//传递经度参数
//                opt.put('wd', mov.point.y);//传递维度参数
                opt.sus = function (data) {
