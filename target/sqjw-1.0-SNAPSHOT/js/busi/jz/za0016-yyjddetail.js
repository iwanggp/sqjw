...
//    alert(param.hyid);
    opt.sus = function (data) {
        if (data.csdata) {
            ...
            alertMsg.confirm("尚未添加该医院的监督信息！是否添加", {okCall: function () {//提示信息   
                    $("#yyjd", navTab.getCurrentPanel()).trigger("click");//激发一次查询按钮的点击，实现了页面的刷新
                }});
        }  
    };
    $.ajax(opt);
