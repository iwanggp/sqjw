...
(function () {
    ...
    // 查看监督
    $('#yyjd', $page).on('click', function () {
        var rowData = $(this).getRow();
        if (rowData) {
            sessionStorage.unionyg = JSON.stringify(rowData);//放到session中暂存一段时间
            $.pdialog.open('page/za/za0016-yyjd.html', 'add_za_yyjd', rowData.mc + "监督检查表",
        ...
        $('#ssxx', $page).cutPage({
            ...
            setTimeout(function () {
                $(".jd-link").unbind("click").bind("click", function (e) {
                    $.pdialog.open('page/za/za0016-yyjddetail.html', 'za_yyjd_detail', $(this).attr('hymc') + "监督检查表",
                            {"width": 1300, "height": 560, mask: true,
