...
(function () {
    ...
    // 查看违章情况
    $('#schooljd', $page).on('click', function () {
        var rowData = $(this).getRow();
        if (rowData) {
            sessionStorage.unionyg = JSON.stringify(rowData);//放到session中暂存一段时间
            $.pdialog.open('page/za/za0015-schooljd.html', 'add_za_schooljd', $(this).attr('hymc') + "监督检查表",
                    {"width": 1180, "height": 560, mask: true,
        ...
        $('#ssxx', $page).cutPage({
            ...
            setTimeout(function () {
                $(".jd-link").unbind("click").bind("click", function (e) {
                    $.pdialog.open('page/za/za0015-schooljddetail.html', 'za_schooljd_detail', "学校监督检查表",
                            {"width": 1180, "height": 560, mask: true,
