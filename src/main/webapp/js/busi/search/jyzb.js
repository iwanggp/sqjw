...
//    getResult();
    ...
    $('#jgjd', $page).on('click', function () {
        ...
        if (rowData) {
            sessionStorage.unionyg = JSON.stringify(rowData);//放到session中暂存一段时间
            $.pdialog.open('page/za/za0020-jyzbjd.html', 'add_za_jyzbjd', rowData.mc + "监督检查表",
        ...
        $('#ssxx', $page).cutPage({
            ...
            setTimeout(function () {
                $(".jd-link").unbind("click").bind("click", function (e) {
                    $.pdialog.open('page/za/za0020-jyzbjddetail.html', 'za_jyzbjd_detail', $(this).attr('hymc') + "监督检查表",
                            {"width": 1300, "height": 560, mask: true,
